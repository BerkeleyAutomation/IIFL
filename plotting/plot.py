...
def smooth_array(array, window_size):
    ...
    for i in range(len(array)):
        z = np.ones(len(array[i]))
        smoothed_x = np.convolve(array[i],y,'same') / np.convolve(z,y,'same')
        array[i] = smoothed_x
    return array
for i in range(0,len(files),3):
    # if i == 6 or i == 9: # skip offline for rohe
    #     continue
    label = '{}'.format(files[i][files[i].rindex('_',0,files[i].rindex('_'))+1:])
    data = np.array(filedatas[i:i+3])
    if len(sys.argv) == 4:
        data2 = np.array(filedatas2[i:i+3])
        data = 100*(data/(data2+20))
    data = smooth_array(data, 500)
...
if len(sys.argv) == 4:
    ...
    plt.savefig('{}_{}.jpg'.format(KEY, KEY2), bbox_inches='tight')
else:
